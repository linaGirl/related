!function(){
	...
	module.exports = new Class({
		...
		, createSubResourceContainer: function(name){
			this.forEach(function(record){
				if (!type.undefined(record[name])) throw new Error('the subresourcecontainer «'+name+'» does already exist on the resource!');
				else record[name] = [];
			}.bind(this));
		}
	});
