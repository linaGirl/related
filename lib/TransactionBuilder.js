(function() {
    ...
    module.exports = function(database) {
        ...
        var classDefinition = {
            ...
            // the user may specify a pool he wants to execute a transaction on
            , pool: null


            // holds the pointer to the connection with the transaction
            // false -> not initialized
            // object -> active transaction
            // null -> transaction ended
            , _transaction: false
            ...
            , executeQueue: function() {
                if (this._queue && this._queue.length) {
                    if (this._transaction) {
                        ...
                        const currentQuery = this._queue[0];

                        // check if we have to wait until all running queries 
                        // have been executed
                        if (!currentQuery.context.transactionExecutionWait || this.runningQueries === 0) {
                            this.runningQueries++;


                            // remove the first item, the reference is already stored
                            // on the currentQuery variable
                            this._queue.shift();

                            // get the sql
                            this.renderQuery(this._transaction, currentQuery.context).then(() => {


                                // execute
                                return this._transaction.query(currentQuery.context).then((result) => {

                                    this.runningQueries--;
                                    if (this.runningQueries === 0) this.executeQueue();

                                    // return
                                    currentQuery.resolve(result);
                                });
