!function(){
	...
	module.exports = new Class({
			...
			this._db.describe(function(err, databases){
				if (err) callback(err);
					...
					async.each(Object.keys(databases), function(databaseName, next){
						if (this[databaseName]) next(new Error('Failed to load ORM for database «'+databaseName+'», the name isreserved for the orm.').setName('ORMException'));
						else {
						...
						if (err) callback(results.filter(function(x){return type.error(x)})[0]);
						else callback();
					}.bind(this));
				}
			}.bind(this));
		...
		, _initializeDatabase: function(options){
			this._db = new DBCluster({type: options.db.type});
