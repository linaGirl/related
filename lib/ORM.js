!function(){
    ...
    ORM = new Class({
         ...
         */
        , createDatabase: function(config, databaseName, callback) {
            if (callback) this._createDatabase(config, databaseName, callback);
            else {
                return new Promise(function(resolve, reject) {
                    this._createDatabase(config, databaseName, function(err, db) {
                        if (err) reject(err);
                        else resolve(db);
                    }.bind(this));
                }.bind(this));
        ...
        /**
         * private method for createing a new database
         * 
         * @param <object> database config or DBCluster instance
         * @param <string> database name
         * @param <function> optional callback, if not passed a promise is returned
         */
        , _createDatabase: function(config, databaseName, callback) {
             var db;

            if (config.type === 'mysql') return callback();

            if (!(config instanceof DBCluster)) db = new DBCluster({}, this._loadDriver(config.type));
            else db = config;
         ...
         * @param <function> optional callback, if not passed a promeise is returned
         */
        , dropDatabase: function(config, databaseName, callback) {
            if (callback) this._createDatabase(config, databaseName, callback);
            else {
                return new Promise(function(resolve, reject) {
                    this._dropDatabase(config, databaseName, function(err, db) {
                        if (err) reject(err);
                        else resolve(db);
                    }.bind(this));
                }.bind(this));
        ...
        /**
         * private method for dropping database
         * 
         * @param <object> database config or DBCluster instance
         * @param <string> database name
         * @param <function> optional callback, if not passed a promise is returned
         */
        , _dropDatabase: function(config, databaseName, callback) {
            var db;
            ...
            config.hosts.forEach(function(hostConfig) {
                if (hostConfig.database) delete hostConfig.database;
                db.addNode(hostConfig);
            }.bind(this));
         ...
         * @param <function> optional callback, if not passed a promeise is returned
         */
        , dropSchema: function(config, schemaName, callback) {
            if (callback) this._createDatabase(config, databaseName, callback);
            else {
                return new Promise(function(resolve, reject) {
                    this._dropSchema(config, schemaName, function(err, db) {
                        if (err) reject(err);
                        else resolve(db);
                    }.bind(this));
                }.bind(this));
         ...
         * @param <function> optional callback, if not passed a promise is returned
         */
        , _dropSchema: function(config, schemaName, callback) {
            var db;
         ...
         * @param <function> optional callback, if not passed a promeise is returned
         */
        , createSchema: function(config, schemaName, callback) {
            if (callback) this._createSchema(config, databaseName, callback);
            else {
                return new Promise(function(resolve, reject) {
                    this._createSchema(config, schemaName, function(err, db) {
                        if (err) reject(err);
                        else resolve(db);
                    }.bind(this));
                }.bind(this));
        ...
        /**
         * private method for createing a new schema
         * 
         * @param <object> database config or DBCluster instance
         * @param <string> schema name
         * @param <function> optional callback, if not passed a promise is returned
         */
        , _createSchema: function(config, schemaName, callback) {
            var db = config instanceof DBCluster ? config : new DBCluster({}, this._loadDriver(config.type));
