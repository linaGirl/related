(function() {
    ...
    ORM = new Class({
        ...
        , init: function(options, pass, host, db, dbName, rdbmsType) {
            ...
            // all loaded dbs
            Class.define(this, '_databaseNames', Class(new Map()));
        ...
        /**
         * returns the names of all loaded dbs
         */
         ...
         * @param <object> config
         */
        , _getDBClusterInstance: function(config) {log(config);
            const db = new DBCluster({
                  driver: config.type
            ...
            Promise.all(this._dbs.map((db) => {
                ...
                return this._databases[db.databaseName].describe([db.databaseName]).then((databases) => {
                    ...
                    if (this[db.databaseName]) return Promise.reject(new Error('Failed to load ORM for database «'+db.databaseName+'», the name is reserved for the orm.'));
                        ...
                        const dbAccessorName = db.alias || db.databaseName;
                        this._databaseNames.set(dbAccessorName);

                        this[dbAccessorName] = new Database({
                              orm:          this
