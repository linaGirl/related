!function(){
    ...
    module.exports = new Class({
        ...
        , addColumn: function(classDefinition, column) {
            ...
            if (column.type === 'json') {
                // json must be compared in another way than scalar values

                definition.set = function(value) {
                    var changed = false;

                    if (type.object(value) || type.array(value)) {
                        if (type.object(this._values[columnName]) || type.array(this._values[columnName])) changed = JSON.stringify(value) !== JSON.stringify(this._values[columnName]);
                        else changed = true;
                    }
                    else if (type.object(this._values[columnName]) || type.array(this._values[columnName])) changed = true;
