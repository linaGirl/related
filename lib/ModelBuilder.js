(function() {
    ...
    let RelatingSet   = require('./RelatingSet');
    

    let debug         = argv.has('dev-orm');
    ...
    module.exports = new Class({
        ...
        init: function(options) {
            ...
            // the current db name
            this.databaseName = this.definition.database.getName();
            this.databaseAliasName = this.definition.database.getAliasName();
        ...
        , build: function() {
            ...
            classDefinition = {
                ...
                , _jsonColumns: Class({}).Writable()
                , _jsonColumnsArray: Class([])
            };
        ...
        , applyExtensions: function(classDefinition) {
            ...
            if (debug) log.info('Found %s extension(s) for the «%s» model ..', extensions.length, this.definition.getName());
            ...
            extensions.forEach(function(extension) {
                ...
                extension.getModelEventListeners().forEach(function(listener) {
                    if (debug) log.debug('Registering «%s» extension event on the «%s» model ...', listener.event, this.definition.getName());
        ...
        , addColumnAccessors: function(classDefinition) {
            ...
            for (let column of this.definition.columns.values()) {
                    ...
                    if (thisContext.mappingMap[mappingName]) {
                        ...
                        if (!this._mappings[mappingId]) {
                            this._mappings[mappingId] = new RelatingSet({
                                ...
                                , databaseAlias: thisContext.databaseAliasName
                                , getDatabase:   thisContext.getDatabase
                                , isMapping:     true
                            });

                            // possible memory leak, somehow? and its not used since we always call save
                            // on the collection
        ...
        , addColumn: function(classDefinition, column) {
            ...
            definition.set = function(value) {
                value = this._prepareTypedInput(columnName, value);
            ...
            // we need this in order to be able to check if there were
            // change son json columns
            if (column.nativeType === 'json') {
                classDefinition._jsonColumns.value[columnName] = null;
                classDefinition._jsonColumnsArray.value.push(columnName);
            }
