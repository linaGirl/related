	...
	module.exports = new Class({
		...
		, setMappingAccessorName: function(mappingName, name) {
			if (!this.Model[name]) {
				...
				this._mappingMap[mappingName].definition.name = name;
		...
		, createModel: function(model, options, additionalProperties){
			...
			properties._genericAccessors = {value: {}};
			...
			Object.keys(this._definition.columns).forEach(function(columnName){
				...
				if (column.mapsTo){
					column.mapsTo.filter(function(mapping){
						...
						if (mapping.useGenericAccessor) properties._genericAccessors.value[mapping.name] = true;
				...
				if (column.belongsTo){
					column.belongsTo.filter(function(belongs){
						...
						if (belongs.useGenericAccessor) properties._genericAccessors.value[belongs.name] = true;
				...
				if (column.referencedModel) {
					...
					if (!column.useGenericAccessor) {
						...
						if (column.useGenericAccessor) properties._genericAccessors.value[column.name] = true;
					}
				}
				else {
