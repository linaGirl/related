!function(){
    ...
    module.exports = new Class({
        ...
        , reload: function(callback, transaction) {
            ...
            var query = {
                  select    : ['*']
                , table      : this._defintion.getTableName()
        ...
        , _save: function(transaction, noReload, callback) {
            ...
            this._emitEvent('beforeSave', transaction, function(err, skipSave) {
                if (err) callback(err);
                    ...
                    this._saveReferences(transaction, noReload, function saveReferencesCallback(err, stopProcess) {
                        var   query
                            , afterSaveCallback;
                ...
                if (err) callback(err);
                    ...
                    async.each(this._changedReferences, function(key, next){
                        ...
                        if (value === null || value === undefined) {
                            ...
                            value.findOne(function findNewReferencedItemByQueryCallback(err, model) {
                                if (err) next(err);
                                else if (!model) {
                                    this[column.name] = null;
                            ...
                            if (!type.function(value)) {
                                next(new Error('Invalid input when saving the reference «'+key+'» on the entity «'+this.getEntityName()+'»!'));
                            }
                            else {
                                if (value.isSaved()) {
                    ...
                    }.bind(this), function referencesSavedCallback(err){
                        if (err) callback(err);
