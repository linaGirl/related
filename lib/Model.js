    ...
    module.exports = new Class({
        ...
        */
        , _emitEvent: function(event, ...args) {
            ...
            const listeners = this._extensionEventListeners[event] ? this._extensionEventListeners[event].concat(this.listener(event)) : this.listener(event).slice(0);
            ...
            if (listeners && listeners.length) {
                ...
                listeners.reduce((promise, currentListener) => {
        ...
        , delete: function(...args) {
            ...
            else promise.then(() => callback()).catch(callback);
        }
        ...
        */
        , save: function(...args) {
            ...
            // invoke the saving process
            const promise = this._prepareSave({dontReload, transaction, args});
            ...
            if (debugHanging) {
                const t = setTimeout(() => {
                    log.error('timeout encountered', 'insert/update', this.getEntityName(), this);
                }, 60000);
        ...
        */
        , _prepareSave: function({dontReload, transaction, args}) {
            ...
            return this._enqueue().then((executeNext) => {
                ...
                if (transaction || this._getDatabase().isTransaction()) {
                    ...
                    // make sure the next callback is invoked!
                    return this._save({transaction, dontReload}).then(() => {
                        executeNext();
                        return Promise.resolve();
                    }).catch((err) => {
                        executeNext();
         ...
         */
        , _save: function({transaction, dontReload}) {
            ...
            try {
                ...
                return this._emitEvent('beforeSave', transaction).then((skipSave) => {
                    if (skipSave) return this._emitEvent('afterSave', transaction);
                        ...
                        // on our record
                        return this._saveReferences({transaction, dontReload}).then((stopProcess) => {
                            if (stopProcess) return Promise.resolve();
                                ...
                                return (this.isFromDatabase() ? this._update.bind(this) : this._insert.bind(this))({transaction}).then((stopProcess) => {
                                    if (stopProcess) return Promise.resolve();
                                        ...
                                        return this._saveDependents({transaction, dontReload}).then(() => {
        ...
        */
         ...
         */
        , _saveDependents: function({transaction, dontReload}) {
            ...
            return this._emitEvent('beforeSaveDependents', transaction).then((skipSave) => {
                if (skipSave) return this._emitEvent('afterSaveDependents', transaction);
                    ...
                    return Promise.all([
                        this._saveMappings({transaction, dontReload}), 
                        this._saveBelongsTo({transaction, dontReload})
                    ]).then(() => {
         ...
         * @param <Object> dontReload -> indicate wheter to reload the models
         */
        , _saveBelongsTo: function({transaction, dontReload}) {
            ...
            return this._emitEvent('beforeSaveBelongsTo', transaction).then((skipSave) => {
                if (skipSave) return this._emitEvent('afterSaveBelongsTo', transaction);
                    ...
                    return Promise.all(Object.keys(this._belongsTo).map((belongsToName) => {
                        ...
                        return this._belongsTo[belongsToName].save(transaction, dontReload);
                    })).then(() => {
         ...
         * @param <Object> dontReload -> indicate wheter to reload the models
         */
        , _saveMappings: function({transaction, dontReload}) {
            ...
            return this._emitEvent('beforeSaveMappings', transaction).then((skipSave) => {
                if (skipSave) return this._emitEvent('afterSaveMappings', transaction);
                    ...
                    return Promise.all(Object.keys(this._mappings).map((mappingName) => {
                            ...
                            return this._mappings[mappingName].save(transaction, dontReload);
                    })).then(() => {
         ...
         * @param <Object> dontReload -> indicate wheter to reload the models
         */
        , _saveReferences: function({transaction, dontReload}) {


            // lifecycle event for extensions and model implementers
            return this._emitEvent('beforeSaveRefernces', transaction).then((skipSave) => {
                    ...
                    return Promise.all(this._changedReferences.map((key) => {
                        ...
                        if (value === null || value === undefined) {
                            ...
                            if (value.isSaved()) {
                                ...
                                return value._save({transaction, dontReload}).then(() => {
                                    this[column.name] = value[column.referencedColumn];
                                    return Promise.resolve();
                                });
                            }
