(function() {
    ...
    module.exports = new Class({
        ...
        , reload: function(callback, transaction) {
            ...
            (transaction && !transaction.hasEnded() ? transaction : this._getDatabase()).executeQuery(context).then((data) => {
                if (!data.length) {
        ...
        , _reloadRelated: function(callback, transaction) {
            Promise.all(Object.keys(this._mappings).map((keyName) => {
                return new Promise((resolve, reject) => {
                    this._mappings[keyName].reload((err) => {
                        if (err) reject(err);
                        else resolve();
                    }, transaction);
                });
        ...
        , _emitEvent: function(event, ...args) {
            ...
            if (listeners && listeners.length) {
                ...
                const callNext = (err, endNow) => {
                    if (err) callback(err);
                    else if (index++ === listeners.length) callback(null, (endNow || dontResume));
                    else if (index > listeners.length) callback(new Error('The related ORM extension event listener for the event «'+event+'» fired twice!'));
                    else {
                        if (endNow) dontResume = true;
                        const promise = listeners[index-1].apply(null, args);

                        // now supporting promises!
                        if (promise && type.function(promise.then) && type.function(promise.catch)) {
                            promise.then((endNow) => callNext(null, endNow)).catch(callNext);
                        }
                    }
                };
        ...
        , _executeDelete: function(callback, transaction, args) {
            ...
            if (transaction || this._getDatabase().isTransaction()) {
                ...
                transaction = this._getDatabase().createTransaction(this._pool);
                ...
                if (!Object.keys(query.filter).length) {
                    ...
                    this._emitEvent('beforeDelete', transaction, function(err, skipDelete) {
                        if (err) callback(err);
                            ...
                            transaction.executeQuery(context).then(() => {
                                this._emitEvent('afterDelete', transaction, callback);
                            }).catch(callback);
                        }
                    }.bind(this));
                }
        ...
        , save: function(...args) {
            ...
            args.forEach((arg) => {
                ...
                else if (type.boolean(arg)) noReload    = arg;
            ...
            if (typeof callback !== 'function') {
                ...
                // return promise
                return new Promise((resolve, reject) => {
                    this._executeSave((err, data) => {
                        if (err) reject(err);
                        else resolve(data);
                    }, noReload, transaction, args);
                });
            } 
            else {
                
                if (debugHanging) {
                    // debugging code for hanging callbacks ...

                    const a = callback;
                    const t = setTimeout(() => {
                        log.error('timeout encountered', this.getEntityName(), this);
                    }, 2000);
                    ...
                    args = args.map((x) => {
                        if (type.function(x)) return callback;
                        else return x;
                    });
                }
        ...
        , _save: function(transaction, noReload, callback) {
            ...
            this._emitEvent('beforeSave', transaction, (err, skipSave) => {
                if (err) callback(err);
                    ...
                    this._saveReferences(transaction, noReload, (err, stopProcess) => {
                        if (err) callback(err);
                            ...
                            // to call
                            let afterSaveCallback = (err, stopProcess) => {
                                if (err) callback(err);
                                else if (stopProcess) callback();
                                else {
                                    this._saveDependents(transaction, noReload, (err) => {
                                        if (err) callback(err);
                            ...
                            if (this.isFromDatabase()) this._update(transaction, afterSaveCallback);
                            else this._insert(transaction, afterSaveCallback);
                        }
                    });
                }
            });
         ...
         */
        , _insert: function(transaction, callback) {
            ...
            this._emitEvent('beforeInsert', transaction, function(err, skipInsert) {
                if (err) callback(err);
                    ...
                    transaction.executeQuery(context).then((result) => {
                        ...
                        if (result !== undefined) {
                            ...
                            // db that are capable of hanlding them return objects
                            if (!type.object(result)) {
                                ...
                                if (result !== null && this._defintion.primaryKeys.length === 1) {
                                    ...
                                    if (this._defintion.primaryKeys.some(function(key) {
                                        ...
                                        throw new Error('Cannot load data for the model «'+this.getEntityName()+'» since mysql doesn\'t support compound primary keys. You need to set the primary key values on the model manually or you have to use a single autoincermenting value as primary instead.');
                                    }
                                }
                            }
                            ...
                            else throw new Error('Insert for the model «'+this.getEntityName()+'» did not return the requested data! please file an issue');

                            // yeh, we're done, emit the afterinsert event
                            this._emitEvent('afterInsert', transaction, callback);
                        }
                        else throw new Error('Unexpected return value while from insert into model «'+this.getEntityName()+'». please file an issue');
                    }).catch(callback);
                }
            }.bind(this));
        }
         ...
         * @param <Function> callback
         */
        , _update: function(transaction, callback) {
            ...
            // emit events first
            this._emitEvent('beforeUpdate', transaction, query, (err, skipUpdate) => {
                if (err) callback(err);
                else if (skipUpdate) this._emitEvent('afterUpdate', transaction, callback);
                else {


                    // we're going to do nothing if there aren't any changes
                    if (!Object.keys(this._getChangedValues()).length) return this._emitEvent('afterUpdate', transaction, callback);
            ...
            this._emitEvent('beforeSaveDependents', transaction, function(err, skipSave) {
                if (err) callback(err);
                    ...
                    Promise.all([new Promise((resolve, reject) => {
                        this._saveMappings(transaction, noReload, (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                    }), new Promise((resolve, reject) => {
                        this._saveBelongsTo(transaction, noReload, (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                    })]).then(() => {
                        this._emitEvent('afterSaveDependents', transaction, callback);
                    }).catch(callback);
                }
            }.bind(this));
        }
        ...
        , _saveBelongsTo: function(transaction, noReload, callback) {
            this._emitEvent('beforeSaveBelongsTo', transaction, function(err, skipSave) {
                if (err) callback(err);
                    ...
                    Promise.all(Object.keys(this._belongsTo).map((belongsToId) => {
                        this._belongsTo[belongsToId].setDebugMode(this._debugMode);
                        this._belongsTo[belongsToId].setPool(this._pool);
                        return new Promise((resolve, reject) => {
                            this._belongsTo[belongsToId].save(transaction, noReload, (err) => {
                                if (err) reject(err);
                                else resolve();
                            });
                        });
                    })).then(() => {
                        this._emitEvent('afterSaveBelongsTo', transaction, callback);
                    }).catch(callback);
                }
            }.bind(this));
        }
        ...
        , _saveMappings: function(transaction, noReload, callback) {
             this._emitEvent('beforeSaveMappings', transaction, function(err, skipSave) {
                if (err) callback(err);
                    ...
                    Promise.all(Object.keys(this._mappings).map((mappingId) => {
                        return new Promise((resolve, reject) => {
                            this._mappings[mappingId].setDebugMode(this._debugMode);
                            this._mappings[mappingId].setPool(this._pool);
                            this._mappings[mappingId].save(transaction, noReload, (err) => {
                                if (err) reject(err);
                                else resolve();
                            });
                        });
                    })).then(() => {
                        this._emitEvent('afterSaveMappings', transaction, callback);
                    }).catch(callback);
                }
            }.bind(this));
        }
        ...
        , _saveReferences: function(transaction, noReload, callback) {
            this._emitEvent('beforeSaveRefernces', transaction, function(err, skipSave) {
                if (err) callback(err);
                    ...
                    Promise.all(this._changedReferences.map((key) => {
                        return new Promise((resolve, reject) => {
                        ...
                        this._emitEvent('afterSaveReferences', transaction, callback);
                    }).catch(callback);
                }
            }.bind(this));
        }
