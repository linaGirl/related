            ...
            if (this._fromDb){
                ...
                if (!Object.keys(query.filter).length) {
                    ...
                    if (!hardDelete && this._defintion.deletedTimestamp) {
                        query.values = {};
                        query.values[this._defintion.deletedTimestamp] = new Date(process.env.ORM_TIMESTAMP_VALUE);
                ...
                if (err) callback(err);
                    ...
                    if (this._fromDb){
                        if (this._changedValues.length) {
                            ...
                            if (this._defintion.updatedTimestamp) query.values[this._defintion.updatedTimestamp] = new Date(process.env.ORM_TIMESTAMP_VALUE);
                        ...
                        if (this._defintion.updatedTimestamp) query.values[this._defintion.updatedTimestamp] = new Date(process.env.ORM_TIMESTAMP_VALUE);
                        if (this._defintion.createdTimestamp) query.values[this._defintion.createdTimestamp] = new Date(process.env.ORM_TIMESTAMP_VALUE);
