(function() {
    ...
    module.exports = new Class({
        ...
        // save changes on the
        , _save: {value: function({transaction, dontReload, stack}) {
            ...
            return this._addQueryItems(transaction).then(() => {
                ...
                return Promise.all(this.map((item) => {
                    if (!item.isFromDatabase() || !item.isSaved()) {
                        ...
                        return item.save(transaction, dontReload);
                    ...
                    return Promise.all([
                        this._deleteRelationRecords({removedRecords, transaction, dontReload, stack}),
                        this._createRelationRecords({addedRecords, transaction, dontReload, stack})
                    ]);
                });
            });
        }}
        ...
        , _createRelationRecords: { value: function({addedRecords, transaction, dontReload, stack}) {
            ...
            return Promise.all(addedRecords.map((record) => {
                if (this.isMapping) {
                    ...
                    return new this._orm[this._definition.model.getDatabaseName()][this._definition.via.model.name](values).setDebugMode(this._debugMode).save(transaction, stack, dontReload).then(() => {
        ...
        , _deleteRelationRecords: {value: function({removedRecords, transaction, dontReload, stack}) {
            ...
            return Promise.all(removedRecords.map((record) => {
                ...
                if (stack) stack.push({entity: this._definition.name, frame: '_deleteRelationRecords: done', values: values});
                return Promise.resolve();
            });
        }}
