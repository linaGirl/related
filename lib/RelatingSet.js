    ...
    module.exports = new Class({
        ...
        , save: {value: function(...args) {
            ...
            const promise = Promise.resolve().then(() => {
                if (transaction) return this._save({transaction, dontReload});
                else {
        ...
        // save changes on the
        , _save: {value: function({transaction, dontReload}) {
            ...
            return this._addQueryItems(transaction).then(() => {
                return Promise.all(this.map((item) => {
                    ...
                    return Promise.all([
                        this._deleteRelationRecords({removedRecords, transaction, dontReload}),
                        this._createRelationRecords({addedRecords, transaction, dontReload})
                    ]);
                });
            });
        }}
        ...
        , _createRelationRecords: { value: function({addedRecords, transaction, dontReload}) {
            return Promise.all(addedRecords.map((record) => {
                if (this.isMapping) {
                    ...
                    return new this._orm[this._definition.model.getDatabaseName()][this._definition.via.model.name](values).setDebugMode(this._debugMode).save(transaction);
                } else return Promise.resolve();
            }));
        }}
        ...
        , _deleteRelationRecords: {value: function({removedRecords, transaction, dontReload}) {
