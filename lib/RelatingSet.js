(function() {
    ...
    module.exports = new Class({
                    ...
                    Promise.all(this.map((item) => {
                        return new Promise((resolve, reject) => {
                            if (!item.isFromDatabase() || !item.isSaved()) {
                                ...
                                item.save(transaction, noReload, (err) => {
                                    if (err) reject(err);
                                    else resolve();
                                });
                            } else resolve();
                        ...
                        this._getChangedRecords(function(err, added, removed) {
                            if (err) callback(err);
                                ...
                                Promise.all([new Promise((resolve, reject) => {
                                    this._deleteRelationRecords(removed, transaction, noReload, (err) => {
                                        if (err) reject(err);
                                        else resolve();
                                    });
                                }), new Promise((resolve, reject) => {
                                    this._createRelationRecords(added, transaction, noReload, (err) => {
                                        if (err) reject(err);
                                        else resolve();
                                    });
        ...
        , _addQueryItems: function(transaction, callback) {
            Promise.all(this._queries.map((config) => {
                return new Promise((resolve, reject) => {
                    ...
                    config.query.find(function(err, set) {
                        if (err) done(err);
                        else if(!set || !set.length) resolve();
                        else {
                            try {
        ...
        , _createRelationRecords: { value: function(addedRecords, transaction, noReload, callback) {
            Promise.all(addedRecords.map((record) => {
                return new Promise((resolve, reject) => {
                    if (this.isMapping) {
                        var values = {};
                        ...
                        new this._orm[this._definition.model.getDatabaseName()][this._definition.via.model.name](values).setDebugMode(this._debugMode).save(transaction, (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                    }
        ...
        , _deleteRelationRecords: {value: function(removedRecords, transaction, noReload, callback) {
            Promise.all(removedRecords.map((record) => {
                return new Promise((resolve, reject) => {
                    if (this.isMapping) {
                        ...
                        transaction[this._definition.via.model.name](values).setDebugMode(this._debugMode).delete((err) => {
                            if (err) reject(err);
                            else resolve();
                        })
                    }
                        ...
                        if (debug) log.info('Relatingset for «'+this._definition.name+'» is deleting a relation record ...');
                        transaction[this._definition.model.name](values).limit(1).setDebugMode(this._debugMode).delete((err) => {
                            if (err) reject(err);
                            else resolve();
                        })
                    }
