(function() {
    ...
    module.exports = new Class({
        ...
        // host to execute the query on, if null
        // the host is determined on a query base
        , host: null
        ...
        /**
         * set the host to execute this query on
         *
         */
        , setHost: function(identifier) {
        ...
        /**
         * group the query by the priaries ids if we got joins but no
        ...
        , applyFilter: function(resource) {
            ...
            // collect the ids for the filter
            let primary = this.primaryKeys[0];
            let ids = this.set.map(item => item[primary]);

            // apply filter
            if (!q.filter) q.filter = {};
            if (!q.filter[this.name]) q.filter[this.name] = {};
            q.filter[this.name][this.primaryKeys[0]] = ORM.in(ids);
        }
