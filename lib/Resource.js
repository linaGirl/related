(function() {
    ...
    module.exports = new Class({
        ...
        // if this query is an aggregate query primary keys 
        // must not be auto added and the output cannot be models
        , _aggregateQuery: false
        ...
        /**
        * mark the query as an aggregate query, this will output
        * raw json objects instead of models and not force add
        * the primary keys
        */
        , isAggregateQuery: function() {
            this.raw = true;
            this._aggregateQuery = true;
        }
        ...
        , prepare: function(skipSelect, isSubQuery) {
            ...
            // we need the primary keys on the root query so that we can get
            // the unique values for creating the model instances in the
            // query compiler. if this is a counting query we're operating
            // on an aggregating query that cannot have the primaries selected
            if (!isSubQuery && !this._countingQuery && !this._aggregateQuery) this.selectPrimaryKeys();
        ...
        , _prepareChildrenForRootResource: function(resource) {
            ...
            // except for the root resource (thats happened already
            // using the prepareRootQuery method).
            // all sub selects need to be filtered in the same way as
        ...
        , prepareSubqueries: function() {
            ...
            if (this.isSelected() && !this.selectApplied()) {
                ...
                if (this.referencedParentColumn) {
                    ...
                    // this is a design flaw and a hack :(
        ...
        /*
         * filter a resource by the ids of my primary key
         * this is called on the root resource only and
        ...
        , applyFilter: function(resource) {
            ...
            switch (configResource.type) {
                ...
                case 'reference':
                    ...
                    const referenceIds = this.set.map(item => item[configResource.referencedParentColumn]).filter(v => v !== null && v!== undefined);
                    q.filter[configResource.targetModel][configResource.targetColumn] = ORM.in(referenceIds);

                    // filter for root resource pks ids too
                    if (!q.filter[this.name]) q.filter[this.name] = {};

                    this.primaryKeys.forEach((key) => {
                        q.filter[this.name][key] = ORM.in(this.set.map(row => row[key]));
                    });
                    break;
                ...
                case 'belongsTo': 
                    ...
                    const belongsToIds = this.set.map(item => item[configResource.referencedParentColumn]).filter(v => v !== null && v!== undefined);
                ...
                case 'mapping':
                    ...
                    const mappingIds = this.set.map(item => item[this.primaryKeys[0]]).filter(v => v !== null && v!== undefined);
