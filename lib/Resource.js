!function () {
    ...
    module.exports = new Class({
        ...
        , init: function (options) {
            ...
            if (debug) log.info('New resource ['+(this.id||this._id)+'] '+this.name.cyan+', has rootFilters: '+ ((!!this._hasRootFilters)+'').yellow+', is selected: '+ ((!!this.query.select.length)+'').yellow+'');
        }
        ...
        , _mergeFilters: function(existing, newFilter) {
            Object.keys(newFilter).forEach(function(key) {
                if (existing[key]) this._mergeFilters(existing[key], newFilter[key]);
        ...
        , getAliasName: function() {
            return this.query.table+this.id;
        }
        ...
        , applyFiltersToParents: function(currentTargetResource, sourceResource, joins, lastTargetResource) {
            var   name   = sourceResource.query.table
        ...
        , applyFilters: function(targetResource) {
            var   name = this.query.table+this.id
                , targetFilter;
