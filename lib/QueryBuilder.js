	...
	module.exports = new Class({
		...
		init: function(options) {
			...
			QB._setProperties = function(){
				...
				this._setProperty('_isTransaction', options.isTransaction);
			}
			...
			Object.keys(options.definition.columns).forEach(function (columnName) {
				...
				if (column.mapsTo.length) {
					column.mapsTo.forEach(function(target){
						...
						if (QB[id]) {
							// duplicate id
							if (target.model.name === options.definition.name) {
								// maps onto self
								id = this.mapperGetterName('other'+target.model.name[0].toUpperCase()+target.model.name.slice(1));
							}
							else throw new Error('Cannot redefine property «'+id+'» for mapping handler from «'+options.definition.name+'» to «'+target.model.name+'» via «'+target.via.model.name+'», ask eventEmitter to fix this!');
						}
