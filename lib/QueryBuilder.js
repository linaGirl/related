	...
	var QueryBuilder = {
		...
		, init: function(options) {
			...
			if (options.rootQuery) this._setProperty('_rootQuery', options.rootQuery);
				...
				this._setProperty('_rootQuery', new Query({
					  from: 	this.tableName
			...
			// the current query
			this._setProperty('_query', options.subJoins || options.query || this._rootQuery);
		...
		, _handleReference: function(columnName, queryParameters, returnTarget){
			var   column 			= this._definition.columns[columnName]
				, modelDefinition 	= column.referencedModel
				, subQuery
				, join;
			...
			join = {
				...
				, target: this._createJoinObject(column.referencedTable, column.referencedColumn)
			}
		...
		, _manageJoins: function(query, parentJoins){
			parentJoins = parentJoins || [];

			query.subresources.forEach(function(subQuery){

			// add my joins
			parentJoins = parentJoins.concat(subQuery.rootJoins);

				if (subQuery.hasFilter){
					// add the joins to the root query
					parentJoins.forEach(function(join){
						this.rootQuery.join.push(join);
					}.bind(this));

					parentJoins = [];
				}

				// yeah, the recursive thingy
				if (subQuery.query.subresources.length) this._manageJoins(subQuery.query, parentJoins);
			}.bind(this));
		}
			...
			query.subresources.forEach(function(subQuery){
				if (subQuery.query.subresources.length) this._removeUnUsedQueries(subQuery.query, (subQuery.selected ? subQuery.query : lastSelected));
					...
					if (!subQuery.selected) {
						log.error('removing kiddies from:');
						log(lastSelected);
