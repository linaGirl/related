(function() {
    ...
    module.exports = new Class({
        ...
        , init: function(options) {
            ...
            // check if we need to select the primarykeys
            if (!options.rootResource && !this._resource.query.select.length) {
        ...
        /**
        * apply filters using the filterbuilder
        */
        , buildFilter(filter) {
            const relatedFilter = new FilterBuilder(this).build(filter);
            //log(filter, relatedFilter);
            this.filter({
                _: relatedFilter
            });
        }
        ...
        /**
         * instruct the query compiler not to use model instances
         * for the data returned.
         */
        , raw: function() {
            this.getrootResource().raw = true;
            return this;
        }
        ...
        /**
         * instructs a transaction to wait for all queries
         * executed prior to this one to be completed before
         * this on eis executed
         */
        , wait: function() {
            this.getrootResource().wait = true;
            return this;
        }
        ...
        /**
         * checks if this model has a coulmn
         *
         * @param {string} ccolumnName the name of the column
         */
        ...
        , _handleReference: function(column, targetModel, queryParameters, returnTarget, isJoinCall, leftJoin) { if (debug) log.info('handleReference, from «'+this.tableName.yellow+'» to «'+targetModel.name.yellow+'» ...');
            ...
            resource = new this._queryBuilders[targetModelName].Resource({
                ...
                , Model                     : this._orm[this.databaseAliasName][targetModelName]
            ...
            if (!isJoinCall) {
                this._queryBuilderInstances[relationName].parseFilter(queryParameters);
                this._queryBuilderInstances[relationName].parseSelect(queryParameters);
            }
        ...
        , _handleBelongsTo: function(column, targetModel, queryParameters, returnTarget, isJoinCall, leftJoin) { if (debug) log.info('handleBelongsTo, from «'+this.tableName.yellow+'» to «'+targetModel.name.yellow+'» ...');
            ...
            if (this._queryBuilderInstances[relationName]) {
                // return cached qb instance
                let instance = this._queryBuilderInstances[relationName];
                // apply filters, selects
                if (!isJoinCall) {
                    instance.parseSelect(queryParameters);
                    instance.parseFilter(queryParameters);
            ...
            // create a child tree node for the querybuilder
            let resource = new this._queryBuilders[targetModelName].Resource({
                  name                      : targetModelName
                , aliasName                 : relationName
                , parentResource            : this._resource
                , Model                     : this._orm[this.databaseAliasName][targetModelName]
                , referencedParentColumn    : column.name
                , referencedParentTable     : this.tableName
                , targetColumn              : targetModel.targetColumn
                , targetModel               : targetModel.model.name
                , joins                     : joins
                , filters                   : filter
                , rootResource              : this._rootResource
        ...
        , _handleMapping: function(column, targetModel, queryParameters, returnTarget, isJoinCall, leftJoin){ if (debug) log.info('handleMapping, from «'+this.tableName.yellow+'» to «'+targetModel.name.yellow+'» ...');
            ...
            let resource = new this._queryBuilders[targetModelName].Resource({
                ...
                , aliasName                 : relationName
                , parentResource            : this._resource
                , Model                     : this._orm[this.databaseAliasName][targetModel.model.name]
                , referencedParentColumn    : column.name
                , referencedParentTable     : this.tableName
                , joins                     : joins
                , filters                   : filter
                , rootResource              : this._rootResource
                , primaryKeys               : targetModel.model.primaryKeys
                , targetColumn              : targetModel.via.fk
                , targetModel               : targetModel.via.model.name
                , type                      : 'mapping'
            ...
            if (type.object(filter)) {
                Object.keys(filter).forEach(function(property){
                    ...
                    if (filter[property] && type.function(filter[property].isQuery)) {
                        ...
                        // store so extensions can access it
                        //this._resource.storeSubquery(filter[property]);
                        ...
                        if (query.filter[tablename][property]) query.filter[tablename][property] = ORM.and(query.filter[tablename][property], filter[property]);
        ...
        , _findColumnForIdentifier: function(propertyName) {
            ...
            Object.keys(this._definition.columns).some(function(colName) {
                let column = this._definition.columns[colName];
        ...
        , _parseSelect: function(query, tableName, select) {
            select.forEach(function(item) {
                if (item === '*') {
                    ...
                    if (type.function(item) && item.isAggregate) this.getresource().isAggregateQuery();
        ...
        /**
         * tell the resource on which pool to execute
         *
         */
        , pool: function(identifier) {
            this.getrootResource().setPool(identifier);
            return this;
        }
        ...
        , find: function() {
            ...
            if (setSize) {
                if (!callback) return Promise.reject(new Error('The «find» method was called with the setSize parameter but without a callback. Please call this method with a callback which will be called for each set!'));
