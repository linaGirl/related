!function() {
    ...
    module.exports = new Class({
        ...
        , leave: function() {
            for (var i = 0, l = arguments.length; i < l; i++) {
                if (type.array(arguments[i])) this.leave.apply(this, arguments[i]);
                else if (type.string(arguments[i])) this._leave[arguments[i]] = true;
                else throw new Error('Expected array or string as argument to the leave method!');
            }
        ...
        , save: asyncMethod(function() {
            ...
            if (this.sourceModelInstance.isDirty()) callback(new Error('Cannot clone a dirty model! Please save your model before cloning it!'));
                ...
                Object.keys(definition.belongsTo).some(function(name) {
                    ...
                    else if (!this._leave[name]) {
                        // we got a problem, cannot clone without knowing what 
                        // to do with each an every belongs to relatio!
                        err = new Error('Cannot clone model «'+definition.name+'», its not clear what to do with the related «'+name+'» model. please set an action using the copy, reasign or leave method!');
                        return true;
                    }
                }.bind(this));
