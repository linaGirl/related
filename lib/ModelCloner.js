    ...
    module.exports = new Class({
        ...
        , copy: function() {
            ...
            for (var i = 0, l = arguments.length; i < l; i++) {
                ...
                else if (type.string(arguments[i])) {
                    this._copy[arguments[i]] = true;
                    addedEntities.push(arguments[i]);
                }
                else if (type.object(arguments[i])) filter = arguments[i];
                else throw new Error('Expected array or string as argument to the copy method!');
            }

            if (filter && addedEntities.length) {
                addedEntities.forEach(function(entity) {
                    this._copy[entity] = filter;
                }.bind(this));
            }
        ...
        , reassign: function() {
            ...
            for (var i = 0, l = arguments.length; i < l; i++) {
                if (type.array(arguments[i])) this.reassign.apply(this, arguments[i]);
                    ...
                    this._reassign[arguments[i]] = true;
                    addedEntities.push(arguments[i]);
                }
                else if (type.object(arguments[i])) filter = arguments[i];
            ...
            if (this.sourceModelInstance.isDirty()) callback(new Error('Cannot clone a dirty model! Please save your model before cloning it!'));
                ...
                Object.keys(definition.belongsTo).some(function(name) {
                    if (this._copy[name]) {
                        ...
                        if (this._copy[name] === true) query.get(name, '*');
                        else query.get(name, '*').filter(this._copy[name]);
                        ...
                        // apply filters if provided
                        if (this._reassign[name] === true) query.get(name, '*');
                        else query.get(name, '*').filter(this._reassign[name]);
